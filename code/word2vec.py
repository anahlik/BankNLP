...
def main():
    ...
    text2 = tokenize(text)
    modelCBOW = gensim.models.Word2Vec(text2, min_count=2, size=100, window=5)
    modelSGRAM = gensim.models.Word2Vec(text2, min_count=2, size=100, window=5, sg=1)
    modelCBOW.wv.save_word2vec_format('../output/model/CBOW.model')
    modelCBOW.wv.save_word2vec_format('../output/model/SGRAM.model')
    # Use the following command (while in the correct directory to generate tensorflow vectors
    # python -m gensim.scripts.word2vec2tensor -i CBOW.model -o testvec
    pass
